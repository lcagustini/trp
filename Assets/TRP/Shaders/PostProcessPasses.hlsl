...
float4 GetSource(float2 screenUV) {
    return SAMPLE_TEXTURE2D_LOD(_PostProcessSource, sampler_linear_clamp, screenUV, 0);
}
...
float4 GetSourceBicubic(float2 screenUV) {
    return SampleTexture2DBicubic(TEXTURE2D_ARGS(_PostProcessSource, sampler_linear_clamp), screenUV, _PostProcessSource_TexelSize.zwxy, 1.0, 0.0);
}
...
float4 BloomPrefilterPassFragment(fragVaryings input) : SV_TARGET {
    float3 color = ApplyBloomThreshold(GetSource(input.screenUV).rgb);
    return float4(color, 1.0);
}

float4 BloomHorizontalPassFragment(fragVaryings input) : SV_TARGET {
    float3 color = 0.0;
    float offsets[] = {
...
float4 BloomVerticalPassFragment(fragVaryings input) : SV_TARGET {
    float3 color = 0.0;
    float offsets[] = {
...
float4 BloomAddPassFragment(fragVaryings input) : SV_TARGET {
...
float4 BloomScatterPassFragment(fragVaryings input) : SV_TARGET {
    float3 lowRes = _BloomBicubicUpsampling ? GetSourceBicubic(input.screenUV).rgb : GetSource(input.screenUV).rgb;
    float3 highRes = GetSource2(input.screenUV).rgb;
    return float4(lerp(highRes, lowRes, _BloomIntensity), 1.0);
}

float4 BloomScatterFinalPassFragment(fragVaryings input) : SV_TARGET {
    float3 lowRes = _BloomBicubicUpsampling ? GetSourceBicubic(input.screenUV).rgb : GetSource(input.screenUV).rgb;
    float4 highRes = GetSource2(input.screenUV);
...
float4 ToneMappingNonePassFragment(fragVaryings input) : SV_TARGET {
    float3 color = GetColorGradedLUT(input.screenUV, false);
    return float4(color, 1.0);
}

float4 ToneMappingACESPassFragment(fragVaryings input) : SV_TARGET {
...
float4 FinalPassFragment(fragVaryings input) : SV_TARGET {
